...
namespace Lean.Touch
{
	...
	public class LeanRotate : MonoBehaviour
	{
		...
		protected virtual void TranslateUI(float twistDegrees, Vector2 twistScreenCenter)
		{
			...
			var twistRotation = Quaternion.Euler(0.0f, 0.0f, - twistDegrees); //remove -
		...
		protected virtual void Rotate(float twistDegrees)
		{
			...
			if (camera != null)
			{
                var axis = transform.InverseTransformDirection(this.transform.up); //change up to forward and this to camera add inverse before transform
